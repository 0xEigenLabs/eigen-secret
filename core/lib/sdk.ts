...
export class SecretSDK {
    ...
    */
            ...
            try {
                ...
                */
                ...
                transactions.push({
                    ...
                    amount: this.formatValue(ctx, txData.amount, ti?.decimals || 18),
                    symbol: ti.symbol,
    ...
    async getL1Balance(ctx: Context, token: string, user: any, decimals: number) {
        let balance: any;
        if (token == ETH) {
        ...
        balance = this.formatValue(ctx, balance, decimals);
    ...
    async getAllBalance(ctx: Context) {
        ...
        for (let [aid, oval] of notesByAssetId) {
            ...
            let val = this.formatValue(ctx, oval, ti?.decimals || 18);
            ...
            totalProfit24h += profit;
            totalPercent += profit / (Number(val) * p24hPrice);
        }
        totalPercent /= notesByAssetId.size;
        let totalReturn = (totalPercent * 100).toFixed(2) + "%";
        let totalProfit24Hour = totalProfit24h.toFixed(2)
    ...
    parseValue(_ctx: Context, value: string, decimals: number = 18) {
        return utils.parseUnits(value, decimals).toBigInt();
    }

    formatValue(_ctx: Context, value: string | bigint, decimals: number = 18) {
